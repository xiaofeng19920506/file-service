...
import { normalizeUserRole } from './permissions.js';
...
export function verifyUserToken(opts: {
  ...
  if (!role) return null;
  // 校验签名用 token 内原始 role；对外返回 normalize 后的 admin|user
  ...
  const payload = `${USER_TOKEN_PREFIX}:${userId}:${email}:${role}:${expiresAtUnix}`;
