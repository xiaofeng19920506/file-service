...
import { DEFAULT_UPLOAD_CHUNK_SIZE, UPLOAD_SESSION_TTL_MS, } from '@file-service/shared';
...
async function concatChunks(session) {
...
export function registerChunkUploadRoutes(app, deps) {
    app.post('/v1/uploads/init', async (request, reply) => {
        ...
        const chunkSize = Math.min(deps.maxUploadBytes, Math.max(256 * 1024, body.chunkSize ?? DEFAULT_UPLOAD_CHUNK_SIZE));
    ...
    app.post('/v1/uploads/:uploadId/chunks/:index', async (request, reply) => {
        ...
        if (!uploaded)
            return reply.code(400).send({ error: 'missing_chunk' });
        const buf = uploaded.buffer;
        if (buf.length === 0)
            return reply.code(400).send({ error: 'empty_chunk' });
        const expectedLen = index === session.totalChunks - 1
...
//# sourceMappingURL=chunk-upload.js.map