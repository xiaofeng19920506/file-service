...
import { createDb, loadApiEnv, runMigrations, createObjectStorage, MERGE_QUEUE_NAME, signDownloadToken, verifyDownloadToken, bullmqConnection, convertToPptx, needsLibreofficeConversion, loadApiKeyConfig, blobStorageKey, blobs, mergeJobs, mergeJobInputs, } from '@file-service/shared';
import { ContentAlreadyExistsError, deleteBlob, persistBlobFromBuffer, updateBlobMetadata, } from './blob-store.js';
import { readMultipartFileBuffer, readUploadMetadata } from './multipart-read.js';
import { registerChunkUploadRoutes, sweepExpiredUploadSessions, } from './chunk-upload.js';
...
async function main() {
    ...
    const getActor = (request) => resolveRequestActor({
        request,
        apiKeyConfig,
    ...
    setInterval(() => {
        sweepExpiredUploadSessions().catch((err) => app.log.error(err, 'upload session sweep failed'));
    }, 30 * 60 * 1000);
    registerHealthRoutes(app, { db, redisUrl: env.REDIS_URL });
    const PPTX_MIME = 'application/vnd.openxmlformats-officedocument.presentationml.presentation';
    const previewPptxCache = new Map();
    app.get('/v1/blobs/:id/preview.pptx', async (request, reply) => {
        const id = request.params.id;
        const [blob] = await db.select().from(blobs).where(eq(blobs.id, id));
        if (!blob)
            return reply.code(404).send({ error: 'not_found' });
        const ext = blob.originalExt ??
        ...
        try {
            ...
            if (previewPptxCache.size > 50) {
                ...
                if (oldest)
                    previewPptxCache.delete(oldest);
            }
            return reply
                .header('Content-Type', PPTX_MIME)
                .header('X-Preview-Converted', 'true')
    ...
    app.get('/v1/blobs', async (request) => {
    ...
    app.get('/v1/blobs/exists', async (request, reply) => {
    ...
    app.patch('/v1/blobs/:id/metadata', async (request, reply) => {
    ...
    app.delete('/v1/blobs/:id', async (request, reply) => {
        const id = request.params.id;
        ...
        for (const key of previewPptxCache.keys()) {
            if (key.startsWith(`${id}:`))
                previewPptxCache.delete(key);
        }
        return { ok: true };
    });
    app.get('/v1/blobs/:id/content', async (request, reply) => {
        const id = request.params.id;
        const [blob] = await db.select().from(blobs).where(eq(blobs.id, id));
        if (!blob)
            return reply.code(404).send({ error: 'not_found' });
        ...
        return reply
            ...
            .header('Content-Disposition', `inline; filename="${blob.originalFilename ?? 'file.pptx'}"`)
            .send(stream);
    });
    app.put('/v1/blobs/:id/content', async (request, reply) => {
        const id = request.params.id;
        const [blob] = await db.select().from(blobs).where(eq(blobs.id, id));
        if (!blob)
            return reply.code(404).send({ error: 'not_found' });
        ...
        const isPptxUpload = mimetype?.includes('presentationml') || extFromName === 'pptx';
        await db
            .update(blobs)
            .set({
    ...
    app.post('/v1/jobs', async (request, reply) => {
        ...
        const [job] = await db
            ...
            .returning();
        await db.insert(mergeJobInputs).values(sorted.map((input, idx) => ({
            jobId: job.id,
            blobId: input.blobId,
            sortOrder: idx,
        })));
        await mergeQueue.add('merge', { mergeJobId: job.id }, { removeOnComplete: true, removeOnFail: false });
    ...
    app.get('/v1/jobs/:id/download', async (request, reply) => {
        ...
        return reply
            ...
            .header('Content-Disposition', 'attachment; filename="merged.pptx"')
            .send(stream);
    });
    app.put('/v1/jobs/:id/output', async (request, reply) => {
        const id = request.params.id;
        const [job] = await db.select().from(mergeJobs).where(eq(mergeJobs.id, id));
        if (!job || job.status !== 'succeeded' || !job.outputKey) {
            return reply.code(404).send({ error: 'not_ready' });
        ...
        await storage.putObject(job.outputKey, buf, 'application/vnd.openxmlformats-officedocument.presentationml.presentation');
        return { jobId: id, sizeBytes: buf.length };
    });
    app.post('/v1/jobs/:id/download-url', async (request, reply) => {
        const id = request.params.id;
        const [job] = await db.select().from(mergeJobs).where(eq(mergeJobs.id, id));
