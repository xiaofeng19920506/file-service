...
import { extractApiKeyFromHeaders, isUploadRateLimitPath, shouldSkipRateLimit, } from '@file-service/shared';
export async function registerRateLimiting(app, env) {
    ...
    app.addHook('preHandler', async (request, reply) => {
        const path = request.url.split('?')[0] ?? request.url;
        if (!isUploadRateLimitPath(request.method, path))
            return;
