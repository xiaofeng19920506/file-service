...
describe('bulletin-sections tree', () => {
  it('keeps pre-family sections flat at depth 0', () => {
    const before = ['cover', 'pre_service', 'scripture', 'worship', 'communion', 'welcome', 'youth_prayer', 'testimony_week', 'message'];
    for (const id of before) {
      expect(BULLETIN_NAV_SECTIONS.find((s) => s.id === id)?.depth).toBe(0);
    }
  });

  it('nests family_time children and announcement grandchildren', () => {
