...
export async function startMergeWorker(): Promise<Worker> {
    ...
    async (job) => {
      ...
      try {
        ...
        await db
          ...
          .where(eq(mergeJobs.id, mergeJobId));
        await notifyJobWebhook(db, mergeJobId, 'job.succeeded', env.WEBHOOK_SECRET).catch(
          (err) => console.error('webhook notify failed', mergeJobId, err),
        );
      } catch (e) {
        await db
          .update(mergeJobs)
          ...
          .where(eq(mergeJobs.id, mergeJobId));
        await notifyJobWebhook(db, mergeJobId, 'job.failed', env.WEBHOOK_SECRET).catch(
          (err) => console.error('webhook notify failed', mergeJobId, err),
        );
        throw e;
      } finally {
