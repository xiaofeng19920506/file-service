...
日常开发：`npm run dev:docker`（Docker 起 Postgres + Redis + **LibreOffice 预览** `:3010` + `:3011`，本机跑 API/Web；`.env` 设 `SOFFICE_PREVIEW_URL=http://localhost:3010,http://localhost:3011`）。
