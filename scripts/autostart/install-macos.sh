...
#
# 用法：
#   ./scripts/autostart/install-macos.sh install-all   # 隧道 + API（推荐）
...
若 API 自启失败且日志含 Operation not permitted，请将仓库移出 Desktop 或给 /bin/bash 完全磁盘访问权限。
EOF
    ;;
  ...
  status) cmd_status ;;
  *)
    echo "用法: $0 {install-all|install-tunnel|install-api|status}" >&2
    exit 1
    ;;
esac
